...
async function DIVINE_SMITE(item) {
  ...
  async function rollDamage(html, event) {
    ...
    const level = (data.level === "pact") ? item.actor.system.spells.pact.level : Number(data.level.at(-1));
    ...
    if (!roll) return;
    const value = item.actor.system.spells[data.level].value - 1;
    return item.actor.update({[`system.spells.${data.level}.value`]: value});
  }
}

async function LAY_ON_HANDS(item) {
  ...
  async function cure() {
    await ChatMessage.implementation.create({
      speaker: ChatMessage.implementation.getSpeaker({actor: item.actor}),
      content: `${item.actor.name} cures a disease or poison.`
    });
